...
import { getAllPostBySearch, getAllTagsBySearch, getAllCatesBySearch, getAllPostsByTags, getAllPostsByCate } from '@/utils/articleUtils'
...
export function useSearch() {
  ...
  const searchArticleItems = computed(() => {
    ...
    if (route.name === 'BlogTags') {
      ...
      searchPlaceHolder.value = routeTags.length === 0 ? '在所有标签的文章中进行搜索' : `在标签含有 ${routeTags.join(',')} 的文章中搜索`
      if (routeTags.length !== 0) {
        return getAllPostBySearch(getAllPostsByTags(allContents, routeTags), searchKeyWord.value)
      }
    }
    if (route.name === 'BlogCate') {
      const routeCate = route.query?.cate as string || ''
      searchPlaceHolder.value = !routeCate.trim() ? '在所有分类的文章中进行搜索' : `在分类为 ${routeCate} 的文章中搜索` 
      if (routeCate.trim()) {
