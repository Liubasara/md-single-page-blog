...
async function generateArticleToJsTask() {
  ...
  return src(allArticlePaths, { base: ARTICLE_PATH }).pipe(markdownToJSON(marked)).pipe(dest(TARGET_PATH))
}
...
async function generateArticleImgTask() {
  ...
  return src(allArticleImgPaths, { base: ARTICLE_PATH }).pipe(dest(TARGET_PATH))
}
